...
function initFilters() {
    ...
    // Адаптация для экранов <=425px: переключение в режим колонки
    const mediaQuery = window.matchMedia("(max-width: 425px)");
    function handleColumnLayout(e) {
        if (e.matches) {
            // Для каждого theme-group добавляем класс для вертикального отображения
            themeGroups.forEach(group => {
                group.classList.add("column-layout");
            });
        } else {
            themeGroups.forEach(group => {
                group.classList.remove("column-layout");
            });
        }
    }
    // Вызовем сразу и подпишемся
    handleColumnLayout(mediaQuery);
    mediaQuery.addEventListener("change", handleColumnLayout);
}
